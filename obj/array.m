classdef array < handle
    ...
    methods
        ...
        function steer(obj,th,ph,inplace)
            % STEER Rotates the array in 3-D space toward some azimuth and
            % elevation.
            %
            % Usage:
            %  STEER(th)
            ...
            %
            % Args:
            %  th: azimuth direction (radians)
            %  ph: (optional) elevation direction; if not passed, 0 is used
            %  inplace: (optional) a boolean indicating if the rotation
            %  should made about the true origin (0,0,0) (false) or the
            %  array's center (true); if not passed, false is used
            if isempty(th)
            ...
            if nargin < 4 || isempty(inplace)
                inplace = false;
            end
            rot_x = ph;
            rot_y = 0;
            rot_z = -th;
            obj.rotate(rot_x,rot_y,rot_z,inplace);
        end
        ...
        function [x,az,el] = get_array_pattern_azimuth(obj,el,N,full)
            ...
            x = zeros(N,1);
            for i = 1:N
                x(i) = obj.get_array_gain(az(i),el); %/obj.get_array_gain(0,0);
            end
        end
        ...
        function show_array_pattern_azimuth(obj,ax,el,N,full)
            ...
            [xx,th,el] = obj.get_array_pattern_azimuth(el,N,full);
            plot(ax,th*180/pi,10*log10(abs(xx)/16384),'-k'); 
            grid(ax,'on');
        ...
        function show_array_pattern_elevation(obj,ax,az,N,full)
            ...
            %plot(ax,ph*180/pi,10*log10(abs(xx)),'-k'); 
            grid(ax,'on');
